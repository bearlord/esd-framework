...
use ESD\Yii\Clickhouse\ColumnSchema;
...
class MigrationDataCommand extends BaseObject
{
    ...
    private function getTotalRows()
    {
        ...
        return (new \ESD\Yii\Db\Query())->from($this->sourceTable)->limit(1)->count('*', $this->sourceDb);
    }
    ...
    private function getRows($offset)
    {
        ...
        return (new \ESD\Yii\Db\Query())->from($this->sourceTable)
    ...
    private function checkTableSchema()
    {
        if (!$this->_schema = $this->storeDb->getTableSchema($this->storeTable, true)) {
            throw new \ESD\Yii\Base\Exception('ClickHouse: table `' . $this->storeTable . '` not found');
        }
        ...
        if (count($columnsNotFound) > 0) {
            throw new \ESD\Yii\Base\Exception('ClickHouse: table `' . $this->storeTable . '` columns not found  (' . implode(',', $columnsNotFound) . ')');
        }
