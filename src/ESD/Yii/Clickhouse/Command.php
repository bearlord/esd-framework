...
class Command extends BaseCommand
{
    ...
    protected function queryInternal($method, $fetchMode = null)
    {
        ...
        Yii::info($rawSql, 'ESD\Yii\Clickhouse\Command::query');

        if ($method !== '') {
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheDependency);
            if (is_array($info)) {
                /* @var $cache \ESD\Yii\Caching\Cache */
                $cache = $info[0];
                $cacheKey = [
                    __CLASS__,
                    $method,
                ...
                if (is_array($result) && isset($result[0])) {
                    Yii::trace('Query result served from cache', 'ESD\Yii\Clickhouse\Command::query');
        ...
        $token = $rawSql;
        try {
            Yii::beginProfile($token, 'ESD\Yii\Clickhouse\Command::query');
            ...
            Yii::endProfile($token, 'ESD\Yii\Clickhouse\Command::query');
        } catch (\Exception $e) {
            Yii::endProfile($token, 'ESD\Yii\Clickhouse\Command::query');
        ...
        if (isset($cache, $cacheKey, $info)) {
            ...
            Yii::trace('Saved query result in cache', 'ESD\Yii\Clickhouse\Command::query');
        }
     ...
     * @param $response \ESD\Yii\Clickhouse\HttpClient\Response
     * @throws Exception
     */
    ...
    {
        $categoryLog = 'ESD\Yii\Clickhouse\Command::batchInsertFiles';
        if ($columns === null) {
    ...
    {
        $categoryLog = 'ESD\Yii\Clickhouse\Command::batchInsertFilesDataSize';
        if ($columns === null) {
            $columns = $this->db->getSchema()->getTableSchema($table)->columnNames;
        }
        $sql = 'INSERT INTO ' . $this->db->getSchema()->quoteTableName($table) . ' (' . implode(', ', $columns) . ')' . ' FORMAT ' . $format;
